...
var mem = function mem(config) {
  var context = {};
  context.gid = config.gid || 'all';
  context.hash = config.hash || id.naiveHash;
  var distribution = global.distribution; // if (!distribution) {
  ...
  return {
    ...
    reconf: function reconf(originalGroup, callback) {
      ...
      distribution.local.groups.get(context.gid, function (e, v) {
        ...
        distribution[context.gid].mem.get(null, function (e, v) {
          ...
          var needReconf = 0; // iterate every key
          ...
          v.forEach(function (key) {
            ...
            if (oldHash !== newHash) {
              ...
              distribution.local.comm.send(message, oldRemote, function (e, value) {
                ...
                var message = [value, newKeyWithGid];
                var newRemote = {
                  node: {
                    ip: newNode.ip,
                    port: newNode.port
